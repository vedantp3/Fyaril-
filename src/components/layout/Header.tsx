
import React from 'react';
import { Search } from 'lucide-react';
import { Input } from "@/components/ui/input";
import { Link, useLocation } from 'react-router-dom';

const Header = () => {
  const location = useLocation();
  const path = location.pathname;
  
  // Determine which user type is active based on the current path
  const userTypes = [
    { name: 'Sellers', path: '/sellers' },
    { name: 'B2B', path: '/b2b' },
    { name: 'B2C', path: '/b2c' },
    { name: 'Partners', path: '/partners' }
  ];
  
  return (
    <header className="w-full bg-gradient-to-r from-fyaril-blue to-fyaril-teal py-4 px-6 sticky top-0 z-50">
      <div className="container mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
        <div className="flex items-center">
          <Link to="/">
            <h1 className="text-white text-2xl font-bold">Fyaril Help Center</h1>
          </Link>
        </div>
        
        {/* User type navigation for mobile/tablet */}
        <div className="md:hidden user-type-nav">
          {userTypes.map((type) => (
            <Link 
              key={type.name}
              to={type.path} 
              className={`user-type-nav-item ${path.startsWith(type.path) ? 'active' : ''}`}
            >
              {type.name}
            </Link>
          ))}
        </div>
        
        <div className="relative w-full max-w-md">
          <div className="relative">
            <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400" size={18} />
            <Input 
              placeholder="Search for help..." 
              className="pl-10 pr-4 py-2 rounded-full border-none bg-white/90 text-gray-800 w-full focus:ring-2 focus:ring-white/50 focus:outline-none" 
            />
          </div>
        </div>
      </div>
      
      {/* Desktop user type navigation with beautiful highlight for current section */}
      <div className="hidden md:flex justify-center mt-4">
        <div className="user-type-nav">
          {userTypes.map((type) => (
            <Link 
              key={type.name}
              to={type.path} 
              className={`user-type-nav-item ${path.startsWith(type.path) ? 'active' : ''}`}
            >
              {type.name}
            </Link>
          ))}
        </div>
      </div>
    </header>
  );
};

export default Header;
